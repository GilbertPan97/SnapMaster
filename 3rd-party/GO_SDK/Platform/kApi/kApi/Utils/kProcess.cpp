...
kFx(kStatus) kProcess_VRelease(kProcess process)
{
    ...
    {
        //We inherited this behavior from a previous implementation. Ideally, we shouldn't terminate the 
        //process and just wait for it to exit gracefully. If that means that the caller is stuck waiting
        //for a process that never exits, that's really their problem (it's a bug in their implementation).
        //We might probably remove this section one day. For now, we'll just keep the original behavior
        //and print the nag message below.

        if (obj->exitCode == k64S_MAX && kProcess_IsValid(process) && !obj->isTerminated)
        {
            kLogf("kProcess: Destroying process before it exited.");
            kLogf("kProcess: In the future, your code may hang (indefinitely) because of this.");
            kLogf("kProcess: Please fix your implementation!");
            kProcess_Terminate(process);
        }
    }
...
kFx(kStatus) kProcess_LegacyWaitImpl(kProcess process, k64u timeout, k64s* exitCode)
{
    kObj(kProcess, process);
    kStatus status;
    ...
    status = kProcess_Wait(process, timeout);

    if (status == kERROR_TIMEOUT)
    {
        kCheck(kProcess_Terminate(process));
    }
    ...
    if (!kIsNull(exitCode))
    {
        *exitCode = kProcess_ExitCode(process);
    }
